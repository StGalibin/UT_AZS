...
Процедура ЗагрузитьДанные() Экспорт
	...
	Если НайденныеФайлы.Количество() <> 0 Тогда
		...
		Для каждого Файл Из НайденныеФайлы Цикл
			...
			ОбработатьФайл (Файл, ТаблицаДанныхЗагрузки, КэшДанных, СтруктураНастроек);
...
СтруктураНастроек.Вставить("РазделятьОперацииПоОрганизациям", Константы.Топаз_ОперацииПоНесколькимОрганизациям.Получить());
СтруктураНастроек.Вставить("Организация", ? (СтруктураНастроек.РазделятьОперацииПоОрганизациям, Справочники.Организации.ПустаяСсылка(), Константы.Топаз_ОсновнаяОрганизация.Получить()));
...
Процедура УстановитьИдентификаторыОпераций(ТаблицаДанныхЗагрузки)
	...
	Для каждого СтрокаДанных Из ТаблицаДанныхЗагрузки Цикл
		...
		ИдентификаторОперации = ? (Константы.Топаз_ОперацииПоНесколькимОрганизациям.Получить(), СтрокаДанных.ПрефиксОрганизации,"");
...
Функция СформироватьСтруктуруТаблицыДанныхЗагрузки()
	...
	ТаблицаДанныхЗагрузки.Колонки.Добавить("ПрефиксОрганизации", Новый ОписаниеТипов (
										   	"Строка",
											   ,
											   ,
											   ,
	...
	ТаблицаДанныхЗагрузки.Колонки.Добавить("ОформлениеОперации", Новый ОписаниеТипов ("СправочникСсылка.Топаз_ОформлениеОпераций"));
	ТаблицаДанныхЗагрузки.Колонки.Добавить("КодОформленияОперации",
										   Новый ОписаниеТипов (
										   	"Строка",
											   ,
											   ,
...
Функция СформироватьКэшДанных()
	...
	КэшДанных.Вставить("ВидыОплатТопаз", ПолучитьОформлениеОперацийТопаз());
...
Функция ПолучитьОформлениеОперацийТопаз()
	
	Запрос       = Новый Запрос;
	Запрос.Текст =
				   "ВЫБРАТЬ
				   |	Топаз_ОформлениеОпераций.Наименование КАК Наименование,
				   |	Топаз_ОформлениеОпераций.ФормаОплаты КАК ФормаОплаты,
				   |	Топаз_ОформлениеОпераций.ВариантОформленияТовародвижения КАК ВариантОформленияТовародвижения,
				   |	Топаз_ОформлениеОпераций.Ссылка КАК Ссылка,
				   |	Топаз_ОформлениеОпераций.Код_Топаз КАК Код,
				   |	Топаз_ОформлениеОпераций.ОплатаПоТопливнымКартам КАК ОплатаПоТопливнымКартам,
				   |	Топаз_ОформлениеОпераций.Организация КАК Организация
				   |ИЗ
				   |	Справочник.Топаз_ОформлениеОпераций КАК Топаз_ОформлениеОпераций";
...
Процедура ОбработатьФайл (Файл, ТаблицаДанныхЗагрузки, КэшДанных, СтруктураНастроек)
	
	Читатель = Новый ЧтениеXML();
	Читатель.ОткрытьФайл(Файл.ПолноеИмя);
	
	СтруктураОбщихДанныхФайла = Новый Структура();
	СтруктураОбщихДанныхФайла.Вставить("ИмяФайла", Файл.ПолноеИмя);
	СтруктураОбщихДанныхФайла.Вставить("РазделятьОперацииПоОрганизациям", СтруктураНастроек.РазделятьОперацииПоОрганизациям);
	СтруктураОбщихДанныхФайла.Вставить("Организация", СтруктураНастроек.Организация);
	СтруктураОбщихДанныхФайла.Вставить("ПрефиксОрганизации", СтруктураНастроек.Организация.Префикс);
			
	Пока Читатель.Прочитать() Цикл
		
		Если (Читатель.Имя = "DataPaket") И (Читатель.ТипУзла = ТипУзлаXML.НачалоЭлемента) Тогда
			СтруктураОбщихДанныхФайла.Вставить("КодСклада", Читатель.ПолучитьАтрибут("AZSCode"));
			
			ПараметрыОтбора = Новый Структура();
			ПараметрыОтбора.Вставить("Код", СтруктураОбщихДанныхФайла.КодСклада);
			СтруктураОбщихДанныхФайла.Вставить("Склад",
										  ПреобразоватьДанные (
										  	КэшДанных.Склады,
											  ПараметрыОтбора));
			
			Если Не СтруктураОбщихДанныхФайла.РазделятьОперацииПоОрганизациям Тогда
				
				ПараметрыОтбора.Очистить();
				ПараметрыОтбора.Вставить("Склад", СтруктураОбщихДанныхФайла.Склад);
				ПараметрыОтбора.Вставить("Организация", СтруктураОбщихДанныхФайла.Организация);
				СтруктураОбщихДанныхФайла.Вставить("КассаККМ",
											  ПреобразоватьДанные (
											  	КэшДанных.КассыККМ,
												  ПараметрыОтбора));
				СтруктураОбщихДанныхФайла.Вставить("Касса",
											  ПреобразоватьДанные (
											  	КэшДанных.КассыККМ,
												  ПараметрыОтбора,
												  "Касса"));
		...
		Если (Читатель.Имя = "Session") И (Читатель.ТипУзла = ТипУзлаXML.НачалоЭлемента) Тогда
			...
			СтруктураОбщихДанныхФайла.Вставить("НомерСмены", Читатель.ПолучитьАтрибут("SessionNum"));
			СтруктураОбщихДанныхФайла.Вставить("НачалоСмены", ДатаВремяИзСтроки(Читатель.ПолучитьАтрибут("StartDateTime")));
			СтруктураОбщихДанныхФайла.Вставить("ОкончаниеСмены", ДатаВремяИзСтроки(Читатель.ПолучитьАтрибут("EndDateTime")));
			ПараметрыОтбора.Очистить();
			ПараметрыОтбора.Вставить("Склад", СтруктураОбщихДанныхФайла.Склад);
			ПараметрыОтбора.Вставить("Код", Читатель.ПолучитьАтрибут("UserName"));
			СтруктураОбщихДанныхФайла.Вставить("Ответственный",
										  ПреобразоватьДанные(
		...
		Если Читатель.Имя = "OutcomeByRetail" И Читатель.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда                  //отпуск нефтепродуктов
			
			КодОформленияОперации =  Читатель.ПолучитьАтрибут("PaymentModeExtCode");
			НаименованиеОформленияОперации = Читатель.ПолучитьАтрибут("PaymentModeName");
			
			ОформлениеОперации =  ПолучитьОформлениеОперации (КэшДанных, КодОформленияОперации, НаименованиеОформленияОперации);
			
			Если ОформлениеОперации.ОплатаПоТопливнымКартам Тогда
				
			Продолжить;
			
			КонецЕсли;
			
			ЭтоСкладскаяОперация = НаименованиеОформленияОперации =  "Тех. прокачка"  Или НаименованиеОформленияОперации = "Переливы" ;
			
			ИмяВидаОперации = ? (ЭтоСкладскаяОперация, "СкладскиеОперации", Читатель.Имя);
			
			Строка = СоздатьИЗаполнитьСтрокуДанных(КэшДанных, СтруктураОбщихДанныхФайла, ТаблицаДанныхЗагрузки, ИмяВидаОперации,ОформлениеОперации);
			
			Если ЭтоСкладскаяОперация Тогда
		...
		Если Читатель.Имя = "IncomeByDischarge" И Читатель.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			КодОформленияОперации =  "8";
			НаименованиеОформленияОперации = "";
			ОформлениеОперации =  ПолучитьОформлениеОперации (КэшДанных, КодОформленияОперации, НаименованиеОформленияОперации);

			Строка = СоздатьИЗаполнитьСтрокуДанных(КэшДанных,
													  СтруктураОбщихДанныхФайла,
													  ТаблицаДанныхЗагрузки,
													  Читатель.Имя,ОформлениеОперации);
		...
		Если Читатель.Имя = "Cash"
			...
			КодОформленияОперации =  Читатель.ПолучитьАтрибут("DocTypeID");
			НаименованиеОформленияОперации = "";
		...
		Если Читатель.Имя = "OutcomeByOffice" И Читатель.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Если не ОформлениеОперации.ОплатаПоТопливнымКартам Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Читатель.Имя = "TechReturn" И Читатель.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Строка = СоздатьИЗаполнитьСтрокуДанных(КэшДанных,
													  СтруктураОбщихДанныхФайла,
													  ТаблицаДанныхЗагрузки,
													  "СкладскиеОперации",ОформлениеОперации);
...
Функция ПолучитьОформлениеОперации(КэшДанных,КодОформленияОперации, НаименованиеОформленияОперации)

	ПараметрыОтбора = Новый Структура();
	
	Если ЗначениеЗаполнено(КодОформленияОперации) Тогда
		
		ПараметрыОтбора.Вставить("Код", КодОформленияОперации);
		
	Иначе
		
		ПараметрыОтбора.Вставить("Наименование", НаименованиеОформленияОперации);
...
КонецФункции // ПолучитьОформлениеОперации()

Функция СоздатьИЗаполнитьСтрокуДанных(КэшДанных, СтруктураОбщихДанных,	 ТаблицаДанныхЗагрузки,	ИмяВидаОперации,  ОформлениеОперации=Неопределено)
	...
	Если не ОформлениеОперации = Неопределено Тогда
	Строка.ОформлениеОперации = ОформлениеОперации;
	Строка.ФормаОплаты = Строка.ОформлениеОперации.ФормаОплаты;
	Строка.КодОформленияОперации =  Строка.ОформлениеОперации.Код_Топаз;
	Строка.ВариантОформленияТовародвижения = Строка.ОформлениеОперации.ВариантОформленияТовародвижения;
КонецЕсли;
Если СтруктураОбщихДанных.РазделятьОперацииПоОрганизациям Тогда
	Строка.Организация = Строка.ОформлениеОперации.Организация;
	Строка.ПрефиксОрганизации = Строка.Организация.Префикс;

	ПараметрыОтбора.Очистить();
	ПараметрыОтбора.Вставить("Склад", Строка.Склад);
	ПараметрыОтбора.Вставить("Организация", Строка.Организация);
	Строка.КассаККМ = ПреобразоватьДанные (
											  	КэшДанных.КассыККМ,
												  ПараметрыОтбора);
	Строка.Касса =ПреобразоватьДанные (
											  	КэшДанных.КассыККМ,
												  ПараметрыОтбора,
												  "Касса");
												  КонецЕсли;
...
Процедура ДобавитьДанныеСкладскихОпераций(Читатель, КэшДанных, Строка, ЭтоВозврат = Ложь)
	...
	//Строка.КодОформленияОперации = Читатель.ПолучитьАтрибут("PaymentModeExtCode");
	//Если ЭтоВозврат Тогда
	//Строка.КодОформленияОперации = "7";
	//КонецЕсли;
	//Если ЗначениеЗаполнено(Строка.КодОформленияОперации) Тогда
	//	
	//	ПараметрыОтбора.Вставить("Код", Строка.КодОформленияОперации);
	//Иначе
	//	
	...
	//Строка.ОформлениеОперации = ПреобразоватьДанные(КэшДанных.ВидыОплатТопаз,
	//									   ПараметрыОтбора)
	//                                       ;
	//Строка.ФормаОплаты = Строка.ОформлениеОперации.ФормаОплаты;
...
Функция ПродажаПоТопливнымКартам(КэшДанных, КодОформленияОперации, НаименованиеВидаОПлаты)
...
Процедура ОбработатьДанныеЗагрузки(ТаблицаДанныхЗагрузки, МассивУспешноОбработанныхФайлов)
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|	ТаблицаДанныхЗагрузки.ОформлениеОперации КАК ОформлениеОперации,
		|	ТаблицаДанныхЗагрузки.КодОформленияОперации КАК КодОформленияОперации,
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ТаблицаДанныхЗагрузки.ОформлениеОперации,
	               |	ТаблицаДанныхЗагрузки.КодОформленияОперации,
	               ...
	               |	ТЗ.ОформлениеОперации КАК ОформлениеОперации,
	               |	ТЗ.КодОформленияОперации КАК КодОформленияОперации,
					...
					Пока ВыборкаВариантОформленияТовародвижения.Следующий() Цикл
						...
						Пока ВыборкаКонтрагент.Следующий() Цикл
							...
							Пока ВыборкаИдентификаторОперации.Следующий() Цикл
								...
								Если ВыборкаИдентификаторОперации.Количество = 0 и ВыборкаИдентификаторОперации.Сумма = 0  Тогда
								Продолжить;
								Иначе
...
Процедура ОбработатьДокумент (ВидДокумента, РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
	...
	ИначеЕсли ВидДокумента = Перечисления.Топаз_ВариантыОформленияТовародвижения.ПриходныйКассовыйОрдер Тогда
		...
		СоздатьДокументыВыбытияДСИзКассыККМ (РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки);
		
	ИначеЕсли ВидДокумента = Перечисления.Топаз_ВариантыОформленияТовародвижения.РасходныйКассовыйОрдер Тогда
		
		СоздатьДокументыВнесенияДСВКассуККМ (РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки);
...
Процедура СоздатьОтчетОРозничныхПродажах(РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.ОформлениеОперации.ФормаОплаты = Перечисления.ФормыОплаты.ПлатежнаяКарта Тогда
...
Процедура СоздатьСписаниеНедостачТоваров(РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
	...
	ЗаполнитьЗначенияСвойств(ДокументОбъект, ВыборкаДетальныеЗаписи);
	ДокументОбъект.СтатьяРасходов = ВыборкаДетальныеЗаписи.ОформлениеОперации.СтатьяРасходов;
...
Процедура СоздатьОприходованиеИзлишковТоваров(РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
	...
	ЗаполнитьЗначенияСвойств(ДокументОбъект, ВыборкаДетальныеЗаписи);
	ДокументОбъект.СтатьяДоходов = ВыборкаДетальныеЗаписи.ОформлениеОперации.СтатьяДоходов;
...
Процедура СоздатьДокументыВыбытияДСИзКассыККМ (РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
...
Процедура СоздатьДокументыВнесенияДСВКассуККМ (РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
	...
	ДокументОбъект.Кассир = ВыборкаДетальныеЗаписи.Ответственный;
	ДокументОбъект.Валюта = ВыборкаДетальныеЗаписи.КассаККМ.ВалютаДенежныхСредств;
	ДокументОбъект.КассаПолучатель       = ВыборкаДетальныеЗаписи.КассаККМ;
	ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствВКассуККМ;
	ДокументОбъект.Дата   = ВыборкаДетальныеЗаписи.Дата + 4;
	ДобавитьИдентификаторОперации(ДокументОбъект, ИдентификаторОперации);
...
Функция СоздатьПередачуДСВОсновнуюКассу (РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи)
	...
	ДокументОбъект.Кассир = ВыборкаДетальныеЗаписи.Ответственный;
	ДокументОбъект.Валюта = ВыборкаДетальныеЗаписи.КассаККМ.ВалютаДенежныхСредств;
	ДокументОбъект.КассаПолучатель       = ДокументОбъект.Касса.ПолучателиПлатежейПриПеремещенииДС[0].ПолучательПлатежа;
