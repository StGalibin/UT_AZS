...
Процедура ЗагрузитьДанные() Экспорт
	...
	Если МассивУспешноОбработанныхФайлов.Количество() >0 Тогда
...
Процедура ОбработатьФайл (Файл, ТаблицаДанныхЗагрузки, КэшДанных)
	...
	Пока Читатель.Прочитать() Цикл
		...
		КонецЕсли;
			...
			Если Читатель.Имя = "OutcomeByOffice" И Читатель.ТипУзла = ТипУзлаXML.НачалоЭлемента и ЭтоПродажаПоТопливнымКартам (КэшДанных,Читатель.ПолучитьАтрибут("PaymentModeExtCode"))Тогда
...
Процедура СоздатьВыбытиеДС (РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
	...
	ДокументОбъект.Записать(РежимЗаписи);
	СоздатьПриходныйОрдер(РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ДокументОбъект.Ссылка);
...
Функция СоздатьРасходныйОрдер(Строка, СуммаДокумента, Реквизиты)

НовыйДокумент = Документы.РасходныйКассовыйОрдер.СоздатьДокумент();
НовыйДокумент.Организация = Реквизиты.Организация;
НовыйДокумент.Касса = Реквизиты.Касса;
НовыйДокумент.КассаККМ = Строка.КассаККМ;
НовыйДокумент.Дата = Строка.НачалоСмены;
НовыйДокумент.Кассир = Строка.Кассир;
НовыйДокумент.Валюта = Реквизиты.Валюта;
НовыйДокумент.СуммаДокумента = СуммаДокумента;
НовыйДокумент.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствВКассуККМ;
НовыйДокумент.Записать(РежимЗаписиДокумента.Проведение);

КонецФункции

Функция СоздатьПриходныйОрдер(РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ДокументВыемка)
