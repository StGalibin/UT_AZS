...
Процедура ОбработатьФайл (Файл, ТаблицаДанныхЗагрузки, КэшДанных)
	...
	Пока Читатель.Прочитать() Цикл
		...
		Если Читатель.Имя = "OutcomeByRetail" И Читатель.ТипУзла = ТипУзлаXML.НачалоЭлемента и не Читатель.ПолучитьАтрибут("PaymentModeName") = "Тех. прокачка" и не 
			ЭтоПродажаПоТопливнымКартам (КэшДанных,Читатель.ПолучитьАтрибут("PaymentModeExtCode"), Читатель.ПолучитьАтрибут("PaymentModeName")) Тогда
				
			Строка = ЗаполнитьДанныеСтрокиПоУмолчанию(КэшДанных, Файл.ПолноеИмя, СтруктураОбщихДанных, ТаблицаДанныхЗагрузки, Читатель.Имя);
		...
		КонецЕсли;
			...
			Если Читатель.Имя = "Cash" И Читатель.ТипУзла = ТипУзлаXML.НачалоЭлемента и не Читатель.ПолучитьАтрибут("DocTypeID")="6"  Тогда
...
Процедура ДобавитьДанныеПродаж(Читатель, КэшДанных, Строка)
	...
	Строка.Количество      = Число (Читатель.ПолучитьАтрибут("Volume"));
	Строка.ВариантОформленияТовародвижения = ОпределитьВариантОформленияТовародвижения(Строка);
...
Функция ОпределитьВариантОформленияТовародвижения(Строка)
	
	Если Строка.ВидОплаты = Справочники.Топаз_ВидыОплат.НайтиПоНаименованию("Переливы") Тогда
	Если Строка.Количество <0 Тогда
	Возврат Перечисления.Топаз_ВариантыОформленияТовародвижения.ОприходованиеИзлишковТоваров;
	КонецЕсли;
    КонецЕсли;
	Возврат Строка.ВидОплаты.ВариантОформленияТовародвижения;
...
Процедура ОбработатьДанныеЗагрузки(ТаблицаДанныхЗагрузки, МассивУспешноОбработанныхФайлов)
	...
	ОбработатьДокумент (ВидДокумента, РежимЗаписи, ВыборкаИдентификаторОперации.ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки);
	КонецЕсли;	
	КонецЦикла;
...
Процедура ОбработатьДокумент (ВидДокумента, РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
	...
	ИначеЕсли ВидДокумента = Перечисления.Топаз_ВариантыОформленияТовародвижения.ОприходованиеИзлишковТоваров Тогда
...
Функция ПолучитьЭквайринговыйТерминалПоКассе (Касса)
	...
	Запрос.УстановитьПараметр("Свойство", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Эквайринговый терминал"));
...
Процедура СоздатьСписаниеНедостачТоваров(РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
	...
	ЗаполнитьЗначенияСвойств(ДокументОбъект, ВыборкаДетальныеЗаписи);
	ДокументОбъект.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоНаименованию("Прочие расходы");
...
Процедура СоздатьОприходованиеИзлишковТоваров(РежимЗаписи, ИдентификаторОперации, ВыборкаДетальныеЗаписи, ЕстьОшибки)
	...
	ЗаполнитьЗначенияСвойств(ДокументОбъект, ВыборкаДетальныеЗаписи);
	ДокументОбъект.СтатьяДоходов = ПланыВидовХарактеристик.СтатьиДоходов.НайтиПоНаименованию("Прочие доходы");
